...
final class PushNowHTTPTests: XCTestCase {

    func testParseStandardErrorEnvelope() {
        let body = #"{"code":"invalid_credentials","message":"bad auth"}"#.data(using: .utf8)!
        let err = PushNowHTTPClient.parseError(status: 401, headers: [:], data: body)
    ...
    func testParseRateLimitEnvelopeWithDetails() {
        ...
        let err = PushNowHTTPClient.parseError(status: 429, headers: [:], data: body)
        XCTAssertEqual(err.code, .rateLimited)
        XCTAssertEqual(err.retryAfterSeconds, 3)
    }

    func testRetryAfterHeaderFallback() {
        let err = PushNowHTTPClient.parseError(status: 429, headers: ["Retry-After": "5"], data: Data())
    ...
    func testUnknownBodyFallsBackToHttpCode() {
        ...
        let err = PushNowHTTPClient.parseError(status: 500, headers: [:], data: body)
        XCTAssertEqual(err.code, PushNowErrorCode(rawValue: "http_500"))
    ...
    func testUnknownServerCodePassesThrough() {
        ...
        let err = PushNowHTTPClient.parseError(status: 400, headers: [:], data: body)
