...
/// ``PushNow`` so unit tests can substitute an in-memory implementation.
protocol PushNowStorage: AnyObject {
    func string(forKey key: String) -> String?
    func setString(_ value: String?, forKey key: String)
}

enum PushNowStorageKeys {
    static let apnsToken    = "_pushnowApnsToken"
    static let deviceToken  = "_pushnowDeviceToken"
    static let auth         = "_pushnowAuth"
}

/// Default keychain-backed storage, mirrored to `UserDefaults` for
/// cheap reads. Keys are namespaced so they can't collide with
/// host-app data.
final class DefaultPushNowStorage: PushNowStorage {
    private let service: String
    private let defaults: UserDefaults

    init(service: String = "me.pushnow.sdk", defaults: UserDefaults = .standard) {
...
final class InMemoryPushNowStorage: PushNowStorage {
