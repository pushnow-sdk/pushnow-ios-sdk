//
//  PushFlyUNDelegateProxy.swift
//  PushFly
//
//  Chaining `UNUserNotificationCenterDelegate` that hooks PushFly into
...
//  before (or after) running PushFly's logic, so existing integrations
...
final class PushFlyUNDelegateProxy: NSObject, UNUserNotificationCenterDelegate {
    private weak var pushFly: PushFly?
    private weak var original: (any UNUserNotificationCenterDelegate)?

    init(pushFly: PushFly, original: (any UNUserNotificationCenterDelegate)?) {
    ...
    func userNotificationCenter(
        ...
        if let original, original.responds(to: #selector(UNUserNotificationCenterDelegate.userNotificationCenter(_:willPresent:withCompletionHandler:))) {
            pushFly?._handleRemoteNotification(userInfo: userInfo) { _ in }
        ...
        // No host delegate: PushFly owns the presentation decision.
        ...
        let options = pushFly?._handleForegroundNotification(userInfo: userInfo, suggestedOptions: fallback) ?? []
    ...
    func userNotificationCenter(
        ...
        pushFly?._handleNotificationTap(userInfo: userInfo)
