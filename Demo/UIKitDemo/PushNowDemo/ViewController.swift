...
//  PushNowDemo (UIKit)
//
//  A minimal screen. All the PushNow integration lives in
...
final class ViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        let footer = UILabel()
        footer.text = "Send this token to your backend. Your backend forwards it to PushNow when sending a notification."
    ...
    private func refreshToken() {
        if let token = PushNow.shared?.deviceToken(), !token.isEmpty {
    ...
    @objc private func copyTapped() {
        guard let token = PushNow.shared?.deviceToken(), !token.isEmpty else { return }
