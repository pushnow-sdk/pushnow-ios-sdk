...
//  own backend; your backend hands it to PushFly when posting a
//  notification.
//
...
struct ContentView: View {
    ...
    private var deviceSection: some View {
        Section {
            ...
            Text("Send this device token to your own backend. Your backend forwards it to PushFly when sending a notification.")
        }
    }
...
final class DemoViewModel: ObservableObject {
    ...
    func refresh() {
        deviceToken = PushFly.shared?.deviceToken()
        isRegistered = PushFly.shared?.isRegistered() ?? false
    }

    func register() {
        isRegistering = true
        PushFly.shared?.onRegister { [weak self] token, error in
